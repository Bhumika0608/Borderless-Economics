...
The state-level model uses a triple difference specification to identify whether PNWER states benefit disproportionately from USMCA:

```
ln(X_spt) = θ(PNWER_s × USMCA_p × Post_t)
          + γ₁(PNWER_s × USMCA_p)
          + γ₂(PNWER_s × Post_t)
          + γ₃(USMCA_p × Post_t)
          + FE_s + FE_p + FE_t + ε_spt
```
...
- `USMCA_p` = 1 if partner is Canada or Mexico, 0 otherwise
- `Post_t` = 1 if year ≥ 2021, 0 otherwise
- `FE_s, FE_p, FE_t` = State, partner, and year fixed effects
...
> *After USMCA implementation, PNWER states' exports to USMCA partners (relative to non-USMCA partners) grew θ percentage points more than non-PNWER states' exports.*
...
|-------------|----------|--------|---------|--------------|
| θ (PNWER × USMCA × Post) | **+58.81%** | [+3.9%, +142.9%] | **0.034** | ** |

Additional statistics: n = 1,200 observations, df = 24, R² = 0.832

*Interpretation: PNWER states experienced a statistically significant heterogeneous effect from USMCA. After controlling for state, partner, and year effects, PNWER states' relative export growth to USMCA partners exceeded that of non-PNWER states by approximately 59 percentage points.*
...
The DDD specification includes three sets of fixed effects:

1. **State fixed effects (FE_s):** Control for time-invariant state characteristics such as geographic location, port infrastructure, and industrial composition.

2. **Partner fixed effects (FE_p):** Control for time-invariant partner characteristics such as market size, distance, and regulatory environment.

3. **Year fixed effects (FE_t):** Control for common macroeconomic shocks affecting all state-partner pairs such as exchange rate movements and global demand fluctuations.
